...
import { ChartDataItemDto } from "@/types/admin/dashboard.types";
import { formatUtcToIctDate } from "@/lib/utils/dateUtils";
...
export function UserGrowthChart({ data }: UserGrowthChartProps) {
  ...
  const chartData = data.map((item) => ({
    ...item,
    formattedDate: formatUtcToIctDate(item.date)
      .split("/")
      .slice(0, 2)
      .join("/"),
    fullDate: formatUtcToIctDate(item.date),
  }));
